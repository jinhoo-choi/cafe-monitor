...
def search_keyword(page, cafe_id, num_id, keyword):
    ...
    try:
        ...
        page.goto(url, wait_until="domcontentloaded", timeout=30000)  # 재시도는 30초
...
def build_card(post, idx, total):
    ...
    # post_dt(KST aware)에서 날짜+시간 모두 추출 → raw_date 의존 제거
    # (네이버 f-e 검색 엔드포인트가 UTC 시각을 내려주는 경우 raw_date 그대로 쓰면 9시간 오차 발생)
    if post_dt:
        ...
        with sync_playwright() as p:
            ...
            for cafe in CAFES:
                ...
                try:
                    ...
                    # page가 crashed 상태일 수 있으므로 about:blank로 리셋 시도
                    try:
            ...
            if all_alerts:
                ...
                all_alerts = sorted(all_alerts, key=lambda x: x.get("score", 0), reverse=True)  # 묶음 후 재정렬
